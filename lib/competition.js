...
class Competition extends EventEmitter {
    ...
    constructor(client, mode, data) {
        ...
        for (const key in DATA) {
            if (
                ...
                this[key] = this._proxy.bind(this, key);
            }
        }
    }
    /**
    ...
    /**
     * Gets a competition interface from competition ID.
     * @param {Client} client Petlja API client instance
     * @param {Number} id ID of the competition
     * @static
     * @returns {Competition} Competition instance from ID
     */
    static fromId(client, id) {
        return new Competition(client, 'id', id);
    }
    /**
